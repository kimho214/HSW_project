...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("토큰 파싱에 실패했습니다:", error);
      alert("사용자 정보를 확인하는 데 실패했습니다. 다시 로그인해주세요.");
      router.push("/login");
      return;
    }

    // 3. 웹소켓 서버(localhost:3001)에 연결합니다.
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        // 🔴 디버깅 로그: 어떤 room_id로 요청하는지 확인
        console.log(`[DEBUG] Fetching history for room_id: ${roomId}`);

        const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000";
        const response = await fetch(`${apiUrl}/messages/${roomId}`);
        const data = await response.json();

        // 🔴 디버깅 로그: 백엔드로부터 받은 데이터 확인
